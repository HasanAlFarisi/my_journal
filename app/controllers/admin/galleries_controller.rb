class Admin::GalleriesController < Admin::BaseController
	...
	def create
		unless params[:admin_gallery_attributes].blank?
			save_attributes = Admin::Gallery.admin_gallery_attributes(params[:admin_gallery_attributes])
		end
		...
		respond_to do |format|
			if @admin_gallery.save
				unless params[:admin_gallery][:photo].blank?
					preloaded = Cloudinary::Uploader.upload(params[:admin_gallery][:photo], :use_filename => true, :public_id => "galleries/#{@admin_gallery.id}", :transformation => [{:width => 600, :crop => :limit}])
				end
	...
	def create_reply
		@create_reply = Admin::GalleryComment.create(gallery_id: params[:gallery_id], body: params[:content], admin_id: params[:admin_id])
		@create_reply.save
		respond_to do |format|
			format.js
		end
